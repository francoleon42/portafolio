...
== Gestión Integral de Flota de Automotores (GIFA) 
Es un sistema desarrollado para optimizar la administración de flotas de transporte, con un enfoque especial en la mejora de la operación de colectivos. Su objetivo principal es incrementar la eficiencia operativa mediante el uso de soluciones tecnológicas innovadoras.

Para lograr esto, se ha implementado un conjunto de funcionalidades diseñadas para elevar la calidad del servicio, automatizando y centralizando los procesos clave, lo que facilita una gestión más ágil y eficaz.
...
Este proyecto refleja competencias técnicas en:

* Desarrollo backend con Spring Boot, JPA/Hibernate y Mysql.
* Implementación de sistemas de análisis de datos y toma de decisiones.
* Gestión de equipos de trabajo utilizando metodologías ágiles (Scrum).
* Manejo de tecnologías modernas como Docker, GitHub Actions y Traccar.
